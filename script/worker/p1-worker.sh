...
TMPDIR=$Path FIL_PROOFS_PARENT_CACHE=$Path/parent WORKER_PATH=$Path/worker  RUST_LOG=filecoin_proofs=info FIL_PROOFS_USE_MULTICORE_SDR=1 FIL_PROOFS_USE_GPU_COLUMN_BUILDER=1 FIL_PROOFS_USE_GPU_TREE_BUILDER=1 lotus-worker-amd-0.10. run --listen=$ip:$port --precommit2=false --commit=false --device=$host 2>&1 | tee -a /data-ceph-2/space-race/log/p1-$host-$time.log
